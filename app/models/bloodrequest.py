...
class BloodRequest(Base):
    ...
    relationship_to_patient = Column(
        Enum(RelationshipType, name="relationship_type"), nullable=False
    )

    matched_donor_id = Column(Integer, ForeignKey("donor_profiles.id"), nullable=True)

    requester = relationship("RequesterProfile", back_populates="blood_requests")

    volunteers = relationship("DonorVolunteer", back_populates="request")

    donation_history = relationship("DonationHistory", back_populates="blood_request")

    # from donation proof table
    proof = relationship("DonationProof", back_populates="blood_request", uselist=False)
