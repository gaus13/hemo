...
def upload_donation_proof(
    request_id: int, proof_file: str, db: Session, current_user: User
):
    # Find donor profile
    donor = (
    ...
    if requester is not None:
        publish_notification(
            requester.user_id,
            "donation_proof.uploaded",
            "Donation proof uploaded",
            "The donor uploaded proof for your blood request.",
...
def verify_donation(
    ...
    if proof is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Donation proof not found."
        )

    # check if already verified(Prevents duplicate verification.)
    if proof.requester_confirmed:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Donation already verified."
        )
    ...
    donor = db.query(DonorProfile).filter(DonorProfile.id == proof.donor_id).first()
    ...
    if donor is not None:
        publish_notification(
            ...
            "donation.verified",
            "Donation verified",
            "Your donation was verified by the requester.",
            request_id=blood_request.id,
